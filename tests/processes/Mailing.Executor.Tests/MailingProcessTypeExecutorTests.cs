...
public class MailingProcessTypeExecutorTests
{
    ...
    public async Task ExecuteProcessStep_ReturnsExpected()
    {
        ...
        A.CallTo(() => _mailingService.SendMails(mailing1.Email, A<IDictionary<string, string>>._, A<IEnumerable<string>>.That.Matches(x => x.Count() == 1 && x.Single() == mailing1.Template)))
            .MustHaveHappenedOnceExactly();
        A.CallTo(() => _mailingService.SendMails("other@email.com", A<IDictionary<string, string>>._, A<IEnumerable<string>>.That.Matches(x => x.Count() == 1 && x.Single() == "test-template")))
            .MustNotHaveHappened();
    }
    ...
    public async Task ExecuteProcessStep_ThrowingTestException_ReturnsExpected()
    {
        ...
        A.CallTo(() => _mailingService.SendMails(A<string>._, A<IDictionary<string, string>>._, A<IEnumerable<string>>._))
            .Throws(error);
