...
public class RegistrationBusinessLogicTest
{
    ...
    public async Task SubmitRegistrationAsync_WithNotExistingDocumentStatusId_ThrowsNotFoundException()
    {
        ...
        A.CallTo(() => _applicationRepository.GetOwnCompanyApplicationUserEmailDataAsync(applicationid, _iamUserId))
            .ReturnsLazily(() => new CompanyApplicationUserEmailData(CompanyApplicationStatusId.SUBMITTED,notExistingCompanyUserId,null));
        A.CallTo(() => _documentRepository.GetDocumentStatusIdAsync(notExistingCompanyUserId,_iamUserId))
            .ReturnsLazily(() => new DocumentStatusIdData(Guid.Empty,null,DocumentStatusId.INACTIVE,false));
        var sut = new RegistrationBusinessLogic(Options.Create(new RegistrationSettings()), _mailingService, null!, null!, null!, null!, _portalRepositories);
    ...
    public async Task SubmitRegistrationAsync_WithUserEmail_SendsMail()
    {
        ...
        var notExistingCompanyUserId = _fixture.Create<Guid>();
        A.CallTo(() => _applicationRepository.GetOwnCompanyApplicationUserEmailDataAsync(applicationId, _iamUserId))
            .ReturnsLazily(() => new CompanyApplicationUserEmailData(CompanyApplicationStatusId.VERIFY, notExistingCompanyUserId, "test@mail.de"));
        A.CallTo(() => _documentRepository.GetDocumentStatusIdAsync(notExistingCompanyUserId,_iamUserId))
            .ReturnsLazily(() => new DocumentStatusIdData(Guid.NewGuid(),null,DocumentStatusId.INACTIVE,false));
        var sut = new RegistrationBusinessLogic(Options.Create(new RegistrationSettings()), _mailingService, null!, null!, null!, null!, _portalRepositories);
    ...
    public async Task SubmitRegistrationAsync_WithoutUserEmail_DoesntSendMail()
    {
        ...
        var notExistingCompanyUserId = _fixture.Create<Guid>();
        A.CallTo(() => _applicationRepository.GetOwnCompanyApplicationUserEmailDataAsync(applicationId, _iamUserId))
            .ReturnsLazily(() => new CompanyApplicationUserEmailData(CompanyApplicationStatusId.VERIFY, notExistingCompanyUserId, null));
        A.CallTo(() => _documentRepository.GetDocumentStatusIdAsync(notExistingCompanyUserId,_iamUserId))
            .ReturnsLazily(() => new DocumentStatusIdData(Guid.NewGuid(),null,DocumentStatusId.INACTIVE,false));
