...
public class ConnectorsBusinessLogicTests
{
    ...
    public ConnectorsBusinessLogicTests()
    {
        ...
        A.CallTo(() => _companyRepository.GetConnectorCreationCompanyDataAsync(A<IEnumerable<(Guid companyId, bool bpnRequested)>>.That.Matches(x => x.Count() == 1 && x.First().companyId == _validCompanyId)))
            .Returns(new List<(Guid CompanyId, string? BusinessPartnerNumber)>
            {
                new(_validCompanyId, "VALID")
            }.ToAsyncEnumerable());
        A.CallTo(() => _companyRepository.GetConnectorCreationCompanyDataAsync(A<IEnumerable<(Guid companyId, bool bpnRequested)>>.That.Matches(x => x.Count() == 2 && x.Any(y => y.companyId == _validCompanyId) && x.Any(y => y.companyId == _validHostId))))
            .Returns(new List<(Guid CompanyId, string? BusinessPartnerNumber)>
            {
                new(_validCompanyId, "VALID"),
                new(_validHostId, "VALID_HOST")
            }.ToAsyncEnumerable());
        A.CallTo(() => _companyRepository.GetConnectorCreationCompanyDataAsync(A<IEnumerable<(Guid companyId, bool bpnRequested)>>.That.Matches(x => x.Count() == 1 && x.Any(y => y.companyId == _invalidCompanyId))))
            .Returns(new List<(Guid CompanyId, string? BusinessPartnerNumber)>().ToAsyncEnumerable());
        A.CallTo(() => _companyRepository.GetConnectorCreationCompanyDataAsync(A<IEnumerable<(Guid companyId, bool bpnRequested)>>.That.Matches(x => x.Count() == 1 && x.Any(y => y.companyId == _companyWithoutBpnId))))
            .Returns(new List<(Guid CompanyId, string? BusinessPartnerNumber)>
            {
        ...
        A.CallTo(() => _connectorsSdFactoryService.RegisterConnector(A<ConnectorInputModel>._, A<string>.That.Matches(x => x == _accessToken), A<string>._))
            .ReturnsLazily(() => Task.CompletedTask);
        A.CallTo(() => _connectorsSdFactoryService.RegisterConnector(A<ConnectorInputModel>._, A<string>.That.Not.Matches(x => x == _accessToken), A<string>._))
