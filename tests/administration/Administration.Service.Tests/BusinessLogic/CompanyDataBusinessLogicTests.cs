...
public class CompanyDataBusinessLogicTests
{
    ...
    public async Task RetriggerSelfDescriptionResponseCreation_CallsExpected()
    {
        ...
        A.CallTo(() => _processStepRepository.IsValidProcess(process.Id, ProcessTypeId.SELF_DESCRIPTION_CREATION, A<IEnumerable<ProcessStepTypeId>>._))
            .Returns((true, new VerifyProcessData(process, Enumerable.Repeat(new ProcessStep(Guid.NewGuid(), ProcessStepTypeId.RETRIGGER_AWAIT_SELF_DESCRIPTION_COMPANY_RESPONSE, ProcessStepStatusId.TODO, process.Id, DateTimeOffset.UtcNow), 1))));
