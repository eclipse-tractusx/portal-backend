...
public class ServiceAccountManagementTests
{
    ...
    public ServiceAccountManagementTests()
    {
        ...
        A.CallTo(() => portalRepositories.GetInstance<IServiceAccountRepository>()).Returns(_serviceAccountRepository);
    ...
    private void SetupDeleteOwnCompanyServiceAccount(bool isDimServiceAccount, Identity? identity = null)
    {
        if (isDimServiceAccount)
        {
            A.CallTo(() => _serviceAccountRepository.GetProcessDataForServiceAccountDeletionCallback(A<Guid>._, A<IEnumerable<ProcessStepTypeId>>._))
