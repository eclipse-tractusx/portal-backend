...
public class ServiceAccountBusinessLogicTests
{
    ...
    public async Task HandleServiceAccountCreationCallback_WithValidOfferSubscription_ExecutesExpected()
    {
        ...
        A.CallTo(() => _serviceAccountRepository.GetProcessDataForServiceAccountCallback(A<Guid>._, A<IEnumerable<ProcessStepTypeId>>._))
        ...
        // Assert
        A.CallTo(() => _serviceAccountRepository.GetProcessDataForServiceAccountCallback(process.Id, A<IEnumerable<ProcessStepTypeId>>.That.Matches(x => x.Count() == 1 && x.Single() == stepToTrigger)))
            .MustHaveHappenedOnceExactly();
