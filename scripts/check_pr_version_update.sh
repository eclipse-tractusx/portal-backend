...
IFS=$'\n' read -d '' -ra updatedVersions < <(git diff --name-only $baseBranch..$currentBranch | grep 'Directory.Build.props')

check_version_update(){
  local directory="$1"
  local updated_name="$2"
  local props_file="src/framework/"$(basename "$directory")"/Directory.Build.props"
  if ! git diff HEAD -- $props_file | grep -q -E '[-+]\s*<VersionPrefix>.+</VersionPrefix>'; then
    version_update_needed+=($directory)
    already_checked_projects+=($directory)
    update_csproj_files_recursive "$updated_name"
  fi

  if git diff HEAD -- $props_file | grep -q -E '[-+]\s*<VersionSuffix>.+</VersionSuffix>'; then
...
IFS=$'\n' read -d '' -ra changedPackages < <(git diff --name-only $baseBranch..$currentBranch | xargs dirname | sort | uniq | grep '^src/framework/.*')
