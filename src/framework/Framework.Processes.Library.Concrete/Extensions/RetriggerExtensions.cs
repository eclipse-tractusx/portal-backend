...
namespace Org.Eclipse.TractusX.Portal.Backend.Framework.Processes.Library.Concrete.Extensions;
...
public static class RetriggerExtensions
{
        ...
        where TProcessStepTypeId : struct, IConvertible
    {
        ...
        var (validProcessId, processData) = await processRepositories.GetInstance<IProcessStepRepository<Process<TProcessTypeId, TProcessStepTypeId>, ProcessStep<TProcessTypeId, TProcessStepTypeId>, TProcessTypeId, TProcessStepTypeId>>().IsValidProcess(processId, processType, Enumerable.Repeat(stepToTrigger, 1)).ConfigureAwait(ConfigureAwaitOptions.None);
        if (!validProcessId)
        {
