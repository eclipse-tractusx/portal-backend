...
public class AppErrorMessageContainer : IErrorMessageContainer
{
    private static readonly IReadOnlyDictionary<int, string> _messageContainer = ImmutableDictionary.CreateRange<int, string>([
        new((int)AppErrors.APP_NOT_EXIST, "App {appId} does not exist")
        ]);

    public Type Type { get => typeof(AppErrors); }
