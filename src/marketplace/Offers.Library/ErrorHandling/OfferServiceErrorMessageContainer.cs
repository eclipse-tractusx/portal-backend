...
public class OfferServiceErrorMessageContainer : IErrorMessageContainer
{
    private static readonly IReadOnlyDictionary<int, string> _messageContainer = new Dictionary<OfferServiceErrors, string> {
                ...
                { OfferServiceErrors.OFFER_STATUS_IN_REVIEW, "{offerType} must be in status {OfferStatusId.IN_REVIEW}" },
                ...
                { OfferServiceErrors.INVALID_CONFIGURATION_ROLES_NOT_EXIST, "invalid configuration, at least one of the configured roles does not exist in the database: {clientRoles.ClientId}" },
