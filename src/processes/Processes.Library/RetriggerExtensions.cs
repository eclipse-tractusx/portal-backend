...
{
    public static async Task TriggerProcessStep(this ProcessStepTypeId stepToTrigger, Guid processId, IPortalRepositories portalRepositories)
    {
        var (processType, nextStep) = stepToTrigger.GetProcessStepForRetrigger();

        var (validProcessId, processData) = await portalRepositories.GetInstance<IProcessStepRepository>().IsValidProcess(processId, processType, Enumerable.Repeat(stepToTrigger, 1)).ConfigureAwait(ConfigureAwaitOptions.None);
        if (!validProcessId)
        {
