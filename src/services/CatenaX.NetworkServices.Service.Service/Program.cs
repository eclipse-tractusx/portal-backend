...
{
    app.UseSwagger(c => c.RouteTemplate = "/api/service/swagger/{documentName}/swagger.{json|yaml}");
    app.UseSwaggerUI(c => {
        c.SwaggerEndpoint($"/api/service/swagger/{VERSION}/swagger.json", $"{TAG} {VERSION}");
        c.RoutePrefix = "api/service/swagger";
    });
}
