    ...
    public async Task<AppDetailsData> GetAppDetailsByIdAsync(Guid appId, string iamUserId, string? languageShortName)
    {
       var app = await _context.Apps.AsNoTracking()
            ...
            .Select(a => new
            {
                ...
                IsPurchased = a.Companies.FirstOrDefault(c => c.CompanyUsers.Any(companyUser => companyUser.IamUser!.UserEntityId == iamUserId)) == null ?
                    (AppSubscriptionStatusId?)null :
                    a.Companies.First(c => c.CompanyUsers.Any(companyUser => companyUser.IamUser!.UserEntityId == iamUserId)).CompanyAssignedApps.SingleOrDefault(x => x.AppId == appId) == null ?
                        null :
                        a.Companies.First(c => c.CompanyUsers.Any(companyUser => companyUser.IamUser!.UserEntityId == iamUserId)).CompanyAssignedApps.Single(x => x.AppId == appId).AppSubscriptionStatusId,
