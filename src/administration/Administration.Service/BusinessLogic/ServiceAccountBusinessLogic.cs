...
public class ServiceAccountBusinessLogic : IServiceAccountBusinessLogic
{
    ...
    public async Task<ServiceAccountDetails> CreateOwnCompanyServiceAccountAsync(ServiceAccountCreationInfo serviceAccountCreationInfos, IdentityData identity)
    {
        ...
        var result = await _portalRepositories.GetInstance<IUserRepository>().GetCompanyIdAndBpnRolesForIamUserUntrackedAsync(identity.CompanyUserId, _settings.ClientId).ConfigureAwait(false);
        if (result == default)
        {
